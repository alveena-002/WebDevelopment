/*
# Fix: cast time literals in seed_demo_clinic rota insertion

## Problem
The rota insertion uses `CASE ... THEN '09:00' ELSE '08:30' END` which
Postgres treats as text, but `shift_start` is a `time` column. The implicit
cast fails: "column shift_start is of type time without time zone but
expression is of type text".

## Fix
Create a replacement function that explicitly casts the CASE result to
`time` using `::time`. All other logic is unchanged.

## Security
No policy changes. SECURITY DEFINER, granted to authenticated.
*/
...
BEGIN
  ...
  SELECT v_clinic_id, s.id, g.d::date,
    (CASE WHEN g.d::date = v_today THEN '09:00'::time ELSE '08:30'::time END),
    '17:30'::time,
  ...
  WHERE s.id IN (v_staff_id,
    (SELECT id FROM clinic_staff WHERE clinic_id = v_clinic_id AND user_id = v_doc1 LIMIT 1),
